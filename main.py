...
def cv_match_temp(_img_bgr, template, threshold) -> bool:
    ...
    # cv.rectangle(img_bgr, top_left, bottom_right, (0, 255, 0), 2)
...
# logging module
lg.basicConfig(level=lg.INFO, format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%Y/%m/%d, %H:%M:%S')
...
while ttl_draw > 0:
    ...
    # img_bgr = cv.imread('resources\\screenshot_1.jpg', 0)
    img_rgb = pag.screenshot()  # pag returns rgb format
    img_bgr = cv.cvtColor(np.asarray(img_rgb), cv.COLOR_RGB2BGR)
    matched = cv_match_temp(img_bgr, TP_SG_WISH, THRESHOLD_1)
    ...
    if matched:
        ...
        time.sleep(rd.uniform(0.25, 0.30))

        safe_check_times = 0
        while True:
            safe_check_times += 1
            pag.click(
                rand_pos(SKIP_POS[0], -5, 5),
                rand_pos(SKIP_POS[1], -5, 5),
                clicks=2, interval=rd.uniform(0.03, 0.08))
            # time.sleep(rd.uniform(0.05, 0.10))

            # pag.screenshot('resources\\screenshot_2.jpg')
            # img_bgr = cv.imread('resources\\screenshot_2.jpg', 0)
            img_rgb = pag.screenshot()  # pag returns rgb format
            img_bgr = cv.cvtColor(np.asarray(img_rgb), cv.COLOR_RGB2BGR)
            matched = cv_match_temp(img_bgr, TP_CLS_BTN, THRESHOLD_2)
            matched2 = cv_match_temp(img_bgr, TP_SG_WISH, THRESHOLD_1)

            if matched and not matched2:
                time.sleep(rd.uniform(0.30, 0.35))
                pag.click(rand_pos(CLOSE_POS[0], -15, 15),
                          rand_pos(CLOSE_POS[1], -15, 15))
                lg.info('Close action complete.')
                # ws.Beep(650, 100)
                break

            if safe_check_times > 5:
                lg.warning('Failsafe Enabled!')
                # sys.exit(0)
                break
